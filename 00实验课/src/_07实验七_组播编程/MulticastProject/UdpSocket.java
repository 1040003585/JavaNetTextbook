...
public class UdpSocket {
	...
	public UdpSocket() {
		try {
			...
			Logger.getLogger(UdpSocket.class.getName()).log(Level.SEVERE, null,
					ex);
		}
	}
	...
	public void recvProcess() {
		...
		while (true) {
			try {
				...
				Logger.getLogger(UdpSocket.class.getName()).log(Level.SEVERE,
						null, ex);
			}
		}
	}
			...
			if (isnumber(strarray[0].substring(0, 1))) {
				if (Integer.parseInt(strarray[0]) == rndCode) {
					for (int i = 0; i < userList.size(); i++) {
						if (strarray[1].equalsIgnoreCase(userList.get(i).ip)
								&& userList.get(i).port == Integer
										.parseInt(strarray[2])) {
							String string = "你指定的组播组（" + strarray[1] + ":"
									+ strarray[2] + "）被占用，请重新申请！";
							byte[] data = string.getBytes();
							dp = new DatagramPacket(data, data.length, dp
					...
					if (!checkm) {
						userList.add(new user(strarray[1], Integer
								.parseInt(strarray[2])));
						// 创建组播组，接收签到
						CreateMulticase(strarray[1], Integer
								.parseInt(strarray[2]));
		...
		new Thread() {
			public void run() {
				try {
					...
					for (int i = 0; i < userList.size(); i++) {
						if (ip.equalsIgnoreCase(userList.get(i).ip)
								&& port.equals(userList.get(i).port)) {
							userList.get(i).xs = received;
						}
						System.out.println(i + "  " + userList.get(i).ip + ":"
								+ userList.get(i).port + "   "
								+ userList.get(i).xs);
					}
					...
					Logger.getLogger(UdpSocket.class.getName()).log(
	...
	public boolean isnumber(String x) {
		...
		if ("1".equalsIgnoreCase(x))
			return true;
		if ("2".equalsIgnoreCase(x))
			return true;
		if ("3".equalsIgnoreCase(x))
			return true;
		if ("4".equalsIgnoreCase(x))
			return true;
		if ("5".equalsIgnoreCase(x))
			return true;
		if ("6".equalsIgnoreCase(x))
			return true;
		if ("7".equalsIgnoreCase(x))
			return true;
		if ("8".equalsIgnoreCase(x))
			return true;
		if ("9".equalsIgnoreCase(x))
			return true;
		return false;
	}
